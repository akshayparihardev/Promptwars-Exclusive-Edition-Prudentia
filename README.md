...
**Prudentia** bridges this massive information asymmetry. It transforms dense contracts into clear, actionable, and legally-grounded insights, serving as an intelligent preparation layer before consulting a professional.

---

## 🧠 Architectural Philosophy: Solving the AI Trust Gap

A generic AI assistant can summarise a pasted contract, but it critically fails in high-stakes legal contexts:

1. **Deterministic Grounding vs. Hallucinated Facts:** 
   Standard LLMs guess legal enforceability based on training data. Prudentia **injects actual Indian statute text** (Indian Contract Act 1872, Specific Relief Act 1963, etc.) deterministically *before* the AI processes the document. The output is grounded in citable law, not probability.
   
2. **Independent Quote Verification:** 
   We do not trust the AI to quote the document accurately. \document_quote_verifier.ts\ runs a strict Levenshtein distance check client-side to mathematically prove that the flagged clause actually exists in the uploaded PDF.

3. **Reactive vs. Proactive Analysis:** 
   Instead of requiring the user to know what questions to ask, Prudentia proactively scans for and flags the 5 most consequential employment clauses (Bonds, Non-competes, IP Assignment, Notice Periods, Probation) automatically.

4. **Consequence Simulation:** 
   Knowing a clause exists isn't enough. Prudentia generates \consequence_scenarios\ with an \outcome_likelihood\—a "what-if simulator" predicting how Indian courts actually handle these disputes.
...
4. **Strict Validation:** The AI output is validated against a rigorous JSON schema via Ajv. If it fails, a self-correcting prompt is automatically dispatched.

### Hackathon Constraints Conquered
- **No Database:** No persistent storage, no vector databases, no data leakage. The document is held purely in serverless memory for a single execution context.
- **Repository Size:** Achieved a highly optimized footprint of ~431 KB, massively under the 10 MB limit.
...
| **Compare contracts** | Numeric clause values dynamically compared against \	ypical_clause_range_reference\ |
| **Highlight risks** | \concern_level\ risk flags strictly grounded in real statute text (\pplicable_law\) |
| **Answer questions** | Grounded keyword Q&A with an explicit, honest \
ot_addressed_in_document\ UI state |
...
## 🛡️ "Assist, Not Replace" Core Philosophy
This is not a disclaimer footer. It is enforced at the architectural level:
- **No legal conclusions stated as fact.** Risk flags use carefully calibrated language: *"may be"*, *"commonly disputed"*, *"uncertain"*.
...
### Prerequisites
- Node.js 18+
- A [Gemini API key](https://aistudio.google.com/app/apikey) (Free tier is fully supported)
...
## ⚖️ Curated Indian Statutes

Prudentia evaluates contracts against a strictly verified, hardcoded dictionary of enforceable Indian laws:
